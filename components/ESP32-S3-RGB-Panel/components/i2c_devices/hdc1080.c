...
esp_err_t hdc1080_reset(void)
{
    ...
    hdc1080_config.val = 0;

    uint8_t data[2];
    data[0] = (uint8_t)(hdc1080_config.val >> 8);
    data[1] = (uint8_t)(hdc1080_config.val & 0xFF);

    return hdc1080_write_bytes(HDC1080_CONFIGURATION_REG, 2, data);
}

esp_err_t hdc1080_set_mode(hdc1080_measure_type_t mode)
{
    hdc1080_config_reg_t hdc1080_config;
    
    // Lecture de la configuration actuelle
    esp_err_t ret = hdc1080_get_config(&hdc1080_config);
    if (ret != ESP_OK) return ret;

    // Ajuste la résolution selon ton mode
    hdc1080_config.resolution = mode;  // vérifie que mode correspond bien aux valeurs attendues (0 à 3)

    // Conversion en buffer uint8_t
    uint8_t data[2] = {
        (uint8_t)(hdc1080_config.val >> 8),
        (uint8_t)(hdc1080_config.val & 0xFF)
    };
...
{
    uint8_t data[2];
    esp_err_t ret = hdc1080_read_bytes(HDC1080_CONFIGURATION_REG, 2, data);

    config->val = (data[0] << 8) | data[1];
